...
public class GuiSettingsModDebug extends GuiScreenTooltip{
	...
	public void initGui() {
		...
		for (int k = 0; k < j; k++) {
			...
			GuiButtonTooltip guismallbutton = new GuiButtonTooltip(k, this.width / 2 - 155 + i % 2 * 160, 
					this.height / 6 - 12 + 24 * (i >> 1), options[k], this.debug.getKeyBinding(options[k])).setTooltip();
