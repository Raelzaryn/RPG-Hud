...
public class GuiSettingsMod extends GuiScreenTooltip {
	...
	public void init() {
		...
		if(this.subSetting.equals("")){
			GuiButtonTooltip guismallbutton = new GuiButtonTooltip(this.width / 2 - 155 + 0 % 2 * 160, this.height / 6 - 14 + 20 * (0 >> 1), "general", new TranslatableTextContent("gui.rpg.general"), button -> {
					GuiButtonTooltip b = (GuiButtonTooltip) button;
					if(b.enumOptions != null)
					    client.setScreen(new GuiSettingsMod(instance, b.enumOptions, new TranslatableTextContent("gui.settings.rpghud")));
			...
			for(HudElementType type : HudElementType.values()){
				...
				if(!settings.isEmpty()) {
					guismallbutton = new GuiButtonTooltip(this.width / 2 - 155 + count % 2 * 160, this.height / 6 - 14 + 20 * (count >> 1), type.name(), new TranslatableTextContent(type.getDisplayName()), button -> {
							GuiButtonTooltip b = (GuiButtonTooltip) button;
							if(b.enumOptions != null){
								this.client.setScreen(new GuiSettingsMod(instance, b.enumOptions, new TranslatableTextContent("gui.settings.rpghud")));
							}
			...
			for(int i = 0; i < settingList.size(); i++){
				if(this.settings.getSetting(settingList.get(i)) instanceof SettingPosition)
				{
					...
					TextFieldWidget xPos = new TextFieldWidgetMod(fontRenderer, ValueType.POSITION, this.width / 2 - 100 + i % 2 * 160, this.height / 6 - 12 + 20 * (i >> 1), 45, 15, new TranslatableTextContent(values[0]));
					xPos.setText(values[0]);
					xPos.setMaxLength(6);
					this.addDrawableChild(xPos);
					fields.add(xPos);

					TextFieldWidget yPos = new TextFieldWidgetMod(fontRenderer, ValueType.POSITION, this.width / 2 - 100 + i % 2 * 160 + 48, this.height / 6 - 12 + 20 * (i >> 1), 45, 15, new TranslatableTextContent(values[1]));
                    ...
                    TextFieldWidget scale = new TextFieldWidgetMod(fontRenderer, ValueType.DOUBLE, this.width / 2 - 100 + i % 2 * 160 + 3,
                            this.height / 6 - 12 + 20 * (i >> 1), 90, 15, new TranslatableTextContent(String.valueOf(this.settings.getDoubleValue(settingList.get(i)))));
				...
				{
					GuiButtonTooltip guismallbutton = new GuiButtonTooltip(this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 14 + 20 * (i >> 1), settingList.get(i), new TranslatableTextContent(this.settings.getButtonString(settingList.get(i))), button -> {
							GuiButtonTooltip b = (GuiButtonTooltip) button;
							if(b.enumOptions != null){
								if(settings.getSetting(b.enumOptions) instanceof SettingColor){
								    client.setScreen(new GuiSettingsModColor(instance, b.enumOptions, new TranslatableTextContent("gui.settings.rpghud")));
								} else {
									settings.increment(b.enumOptions);
									button.setMessage(new TranslatableTextContent(settings.getButtonString(b.enumOptions)));
								}
							}
		...
		this.addDrawableChild(new ButtonWidget(this.width / 2 - 100, this.height / 6 + 168, 200, 20, new TranslatableTextContent("gui.done"), button -> {
