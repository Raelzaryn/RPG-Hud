...
public class GuiSettingsModColor extends GuiScreenTooltip {
	...
	public void init() {
		...
		this.colorCodeField = new TextFieldWidget(client.textRenderer, this.width / 2 - 74, 115, 147, 20, new TranslatableTextContent(Settings.intToHexString(this.color)));
		...
		for(int i = 0; i < 6; i++) {
			this.addDrawableChild(new GuiButtonTooltip(10 + i,this.width / 4 * 3 - 20, 40 + (i * 20), 60, 20, new TranslatableTextContent(colorString[i]), button -> {
					actionPerformed(button);
			}));
		}

		for(int i = 0; i < 6; i++) {
			this.addDrawableChild(new GuiButtonTooltip(16 + i, this.width / 4 * 3 + 60 - 20, 40 + (i * 20), 60, 20, new TranslatableTextContent(colorString[i+6]), button -> {
					actionPerformed(button);
			}));
		}

		this.addDrawableChild(new GuiButtonTooltip(this.width / 2 - 100, this.height / 6 + 168, 125, 20, new TranslatableTextContent("gui.done"), button -> {
				setSettingColor();
				client.setScreen(parent);
		}).setTooltip(I18n.translate("tooltip.done", new Object[0])));
		this.addDrawableChild(new GuiButtonTooltip(this.width / 2 + 24, this.height / 6 + 168, 75, 20, new TranslatableTextContent("gui.cancel"), button -> {
		    client.setScreen(parent);
