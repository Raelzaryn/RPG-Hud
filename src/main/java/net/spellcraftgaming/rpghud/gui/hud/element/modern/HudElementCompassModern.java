...
public class HudElementCompassModern extends HudElementCompassVanilla {
	...
	public void drawElement(GuiGraphicsExtractor gg, float zLevel, DeltaTracker partialTicks, int scaledWidth, int scaledHeight) {
		...
		if (rotation > 0 && rotation <= 100) {
			gg.centeredText( this.mc.font, "W", posX + (50 * swapSides) - (rotation * swapSides), posY + 1, -1);
		}

		if (rotation > 25 && rotation <= 125) {
			gg.centeredText( this.mc.font, ".", posX + (75 * swapSides) - (rotation * swapSides), posY - 2, -1);
		}

		if (rotation > 50 && rotation <= 150) {
			gg.centeredText( this.mc.font, "N", posX + (100 * swapSides) - (rotation * swapSides), posY + 1, this.settings.getBoolValue(Settings.enable_compass_color) ? 0xE60909 : -1);
		}

		if (rotation > 75 && rotation <= 175) {
			gg.centeredText( this.mc.font, ".", posX + (125 * swapSides) - (rotation * swapSides), posY - 2, -1);
		}

		if (rotation > 100 && rotation <= 200) {
			gg.centeredText( this.mc.font, "E", posX + (150 * swapSides) - (rotation * swapSides), posY + 1, -1);
		}

		if (rotation >= 125) {
			gg.centeredText( this.mc.font, ".", posX + (175 * swapSides) - (rotation * swapSides), posY - 2, -1);
		} else if (rotation <= 25) {
			gg.centeredText( this.mc.font, ".", posX - (25 * swapSides) - (rotation * swapSides), posY - 2, -1);
		}

		if (rotation >= 150) {
			gg.centeredText( this.mc.font, "S", posX + (200 * swapSides) - (rotation * swapSides), posY + 1, -1);
		} else if (rotation <= 50) {
			gg.centeredText( this.mc.font, "S", posX - (rotation * swapSides), posY + 1, -1);
		}

		if (rotation >= 175) {
			gg.centeredText( this.mc.font, ".", posX + (225 * swapSides) - (rotation * swapSides), posY - 2, -1);
		} else if (rotation <= 75) {
			gg.centeredText( this.mc.font, ".", posX + (25 * swapSides) - (rotation * swapSides), posY - 2, -1);
		}
		...
		if (this.settings.getBoolValue(Settings.enable_compass_coordinates)) {
			...
			gg.text(this.mc.font, String.valueOf(pos[0]), (posX - 48) * 2, (posY + 12) * 2, -1);
			gg.centeredText( this.mc.font, String.valueOf(pos[1]), posX * 2, (posY + 12) * 2, -1);
			gg.text(this.mc.font, String.valueOf(pos[2]), (posX + 48) * 2 - this.mc.font.width(String.valueOf(pos[2])), (posY + 12) * 2, -1);
			gg.pose().scale(2f, 2f);
		}
	}
