...
public class HudElementArmorDefault extends HudElement {
	...
	public void drawElement(DrawableHelper gui, MatrixStack ms, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		for (int i = 1; level > 0 && i < 20; i += 2) {
			if (i < level) {
				DrawableHelper.drawTexture(ms, left + 48, top - 2, 34, 9, 9, 9);
			} else if (i == level) {
				DrawableHelper.drawTexture(ms, left + 48, top - 2, 25, 9, 9, 9);
			} else if (i > level) {
				DrawableHelper.drawTexture(ms, left + 48, top - 2, 16, 9, 9, 9);
			}
			left += 8;
		}
	}
