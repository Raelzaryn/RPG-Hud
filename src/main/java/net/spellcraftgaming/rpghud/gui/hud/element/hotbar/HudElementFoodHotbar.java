...
public class HudElementFoodHotbar extends HudElement {
	...
	public void drawElement(Gui gui, float zLevel, float partialTicks) {
		...
		if (GameData.isPlayerHungered()) {
			drawCustomBar(posX, height - 26, 200 + offset, 10, stamina / (double) foodMax * 100.0D, -1, -1, this.settings.color_hunger, offsetColorPercent(this.settings.color_hunger, OFFSET_PERCENT));
		} else {
			drawCustomBar(posX, height - 26, 200 + offset, 10, stamina / (double) foodMax * 100.0D, -1, -1, this.settings.color_stamina, offsetColorPercent(this.settings.color_stamina, OFFSET_PERCENT));
		}
