...
public class HudElementDetailsVanilla extends HudElement{
			...
			if(this.settings.show_itemdurability && item.isItemStackDamageable()){
				...
				if((hand == EnumHand.MAIN_HAND ? ModRPGHud.renderDetailsAgain[0] : ModRPGHud.renderDetailsAgain[1]) || 
					...
					for(int y = 0; y < x; y++){
						...
						if(item != null && Item.getIdFromItem(item.getItem()) == 
								Item.getIdFromItem(this.mc.player.getHeldItem(hand).getItem())){
							z += item.getCount();
						}
					}
					if(hand == EnumHand.MAIN_HAND) this.count1 = z;
					else this.count2 = z;
				} else {
					if(hand == EnumHand.MAIN_HAND) z = this.count1;
					else z = this.count2;
				}
	...
	private void drawArrowCount(Gui gui) {
		...
		if(this.settings.show_arrowcount && item != null && this.mc.player.getHeldItemMainhand().getItem() instanceof ItemBow){
			...
			if(ModRPGHud.renderDetailsAgain[2] || 
				...
				if(item.getItem() instanceof ItemTippedArrow) type1 = PotionUtils.getPotionTypeFromNBT(item.getTagCompound());
				for(int y = 0; y < x; y++){
			...
			if(this.settings.reduce_size) GL11.glScaled(0.5D, 0.5D, 0.5D);
			RenderHelper.enableGUIStandardItemLighting();
			if(ItemStack.areItemStacksEqual(this.itemArrow, ItemStack.EMPTY)) this.itemArrow = new ItemStack(Items.ARROW);
