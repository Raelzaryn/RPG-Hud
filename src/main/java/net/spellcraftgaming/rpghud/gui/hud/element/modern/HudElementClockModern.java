...
public class HudElementClockModern extends HudElementClockVanilla {
    ...
    public void drawElement(Gui gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
        ...
        if(this.settings.getStringValue(Settings.clock_time_format) == "time.24")
            gui.drawCenteredString(this.mc.fontRenderer, this.getTime(), xOffset * 2 + 24, 48 + 2 * yOffset, clockColor);
        else
            gui.drawCenteredString(this.mc.fontRenderer, this.getTime(), xOffset * 2 + 28, 48 + 2 * yOffset, clockColor);
