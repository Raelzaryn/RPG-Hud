	...
	public void drawElement(Gui gui, float zLevel, float partialTicks) {
		...
		ItemStack currentItem = this.mc.player.getHeldItemMainhand();
			if(currentItem != null && currentItem.getItem() instanceof ItemFood && this.mc.player.getFoodStats().needFood() && this.settings.show_hunger_preview) {
				...
				drawCustomBar(49, 22, 110, 12, bonusHunger / 20.0D * 100.0D, 0, 0, staminaColor[4], staminaColor[5]);
			}
		if (this.mc.player.isPotionActive(MobEffects.HUNGER)) {
			drawCustomBar(49, 22, 110, 12, stamina / 20.0D * 100.0D, 0, 0, staminaColor[2], staminaColor[3]);
		} else {
			drawCustomBar(49, 22, 110, 12, stamina / 20.0D * 100.0D, 0, 0, staminaColor[0], staminaColor[1]);
		}
