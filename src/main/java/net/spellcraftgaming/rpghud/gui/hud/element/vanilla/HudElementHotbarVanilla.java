	...
	public void drawElement(Gui gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
        if (this.mc.playerController.getCurrentGameType() == GameType.SPECTATOR) {
			...
			GlStateManager.color4f(1.0F, 1.0F, 1.0F, 1.0F);
	         ...
	         int posY =  this.settings.getPositionValue(Settings.hotbar_position)[1] + offset;
	         float f = zLevel;
	         zLevel = -90.0F;
	         gui.drawTexturedModalRect(i - 91, scaledHeight - 22 + posY, 0, 0, 182, 22);
	         gui.drawTexturedModalRect(i - 91 - 1 + this.mc.player.inventory.currentItem * 20, scaledHeight - 22 + posY - 1, 0, 22, 24, 22);
	         if (!itemstack.isEmpty()) {
	            if (enumhandside == EnumHandSide.LEFT) {
	               gui.drawTexturedModalRect(i - 91 - 29, scaledHeight - 23 + posY, 24, 22, 29, 24);
	            } else {
	               gui.drawTexturedModalRect(i + 91, scaledHeight - 23 + posY, 53, 22, 29, 24);
	            }
	         }
