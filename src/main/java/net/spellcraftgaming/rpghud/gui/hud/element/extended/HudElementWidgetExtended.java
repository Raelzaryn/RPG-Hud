...
public class HudElementWidgetExtended extends HudElement {
	...
	public void drawElement(Gui gui, PoseStack ms, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		GuiComponent.blit(ms,posX + (this.settings.getBoolValue(Settings.render_player_face) ? 50 : 26), posY + (this.settings.getBoolValue(Settings.render_player_face) ? 4 : 0), 0, 35, 114, 44);
		if (this.mc.player.getVehicle() instanceof LivingEntity) {
			GuiComponent.blit(ms,posX + (this.settings.getBoolValue(Settings.render_player_face) ? 51 : 23), posY + (this.settings.getBoolValue(Settings.render_player_face) ? 44 : 39), 164, 0, 92, 20);
		}
		...
		if (this.settings.getBoolValue(Settings.render_player_face)) {
			...
			GuiComponent.blit(ms,posX, 3, 214, 20, 26, 38);
