...
public class HudElementHealthModern extends HudElement {
    ...
    public void drawElement(Gui gui, float zLevel, float partialTicks, int scaledHeight, int scaledWidth) {
        ...
        if(this.settings.getBoolValue(Settings.show_numbers_health) && this.settings.getBoolValue(Settings.show_numbers_food)) {
            ...
            gui.drawCenteredString(this.mc.fontRenderer, stringHealth, textPosX * 2 + (this.settings.getBoolValue(Settings.render_player_face) ? 42 : 0) + width + 4,
                    posY * 2 + 12, -1);
            GL11.glScaled(2.0D, 2.0D, 2.0D);
        }

        this.drawTetragon(posX, posX, 3 + posY, 3 + posY, 97, 83, 10, 10, 0xA0000000);
        this.drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY, 89, 79, 6, 6, 0x20FFFFFF);

        if(absorption > 1)
            this.drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY, (int) (89 * ((double) (health + absorption) / (double) (healthMax + absorption))),
        ...
        if(this.mc.player.isPotionActive(MobEffects.POISON))
            this.drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY, (int) (89 * ((double) health / (double) (healthMax + absorption))),
