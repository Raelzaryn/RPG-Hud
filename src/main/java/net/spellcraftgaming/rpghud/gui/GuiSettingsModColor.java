...
public class GuiSettingsModColor extends GuiScreenTooltip {
    ...
    public void init() {
        ...
        this.colorCodeField = new TextFieldWidget(minecraft.fontRenderer, this.width / 2 - 74, 115, 147, 20,
                new TranslationTextComponent(Settings.intToHexString(this.color)));
        ...
        for(int i = 0; i < 6; i++) {
            this.addButton(
                    new GuiButtonTooltip(10 + i, this.width / 4 * 3 - 20, 40 + (i * 20), 60, 20, new TranslationTextComponent(colorString[i]), button -> {
        ...
        for(int i = 0; i < 6; i++) {
            this.addButton(new GuiButtonTooltip(16 + i, this.width / 4 * 3 + 60 - 20, 40 + (i * 20), 60, 20,
                    new TranslationTextComponent(colorString[i + 6]), button -> {
                        actionPerformed(button);
                    }));
        }

        this.addButton(
                new GuiButtonTooltip(this.width / 2 - 100, this.height / 6 + 168, 125, 20, new TranslationTextComponent("gui.done"), button -> {
                    setSettingColor();
                    minecraft.displayGuiScreen(parent);
                }).setTooltip(I18n.format("tooltip.done", new Object[0])));
        this.addButton(
                new GuiButtonTooltip(this.width / 2 + 24, this.height / 6 + 168, 75, 20, new TranslationTextComponent("gui.cancel"), button -> {
                    minecraft.displayGuiScreen(parent);
    ...
    public void render(MatrixStack ms, int mouseX, int mouseY, float partialTicks) {
        ...
        AbstractGui.drawCenteredString(ms, fontRenderer, this.title, this.width / 2, 12, -1);
        AbstractGui.drawCenteredString(ms, fontRenderer, I18n.format("color.red", new Object[0]), this.width / 2, 40 - 9, -1);
        AbstractGui.drawCenteredString(ms, fontRenderer, I18n.format("color.green", new Object[0]), this.width / 2, 65 - 9, -1);
        AbstractGui.drawCenteredString(ms, fontRenderer, I18n.format("color.blue", new Object[0]), this.width / 2, 90 - 9, -1);
        this.colorCodeField.renderButton(ms, mouseX, mouseY, partialTicks);
        AbstractGui.drawCenteredString(ms, fontRenderer, I18n.format("gui.rpg.result", new Object[0]) + ": " + Settings.intToHexString(this.color), this.width / 2,
                141, -1);
