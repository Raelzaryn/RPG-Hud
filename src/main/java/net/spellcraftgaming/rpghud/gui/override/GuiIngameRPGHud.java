...
public class GuiIngameRPGHud extends GuiIngameForge {
    ...
    protected void renderPortal(float partialTicks) {
        ...
        float timeInPortal = this.mc.player.prevTimeInPortal + (this.mc.player.timeInPortal - this.mc.player.prevTimeInPortal) * partialTicks;
     ...
     */
    /*
     * private void drawElementChat(float partialTicks) { if
     * (this.rpgHud.getActiveHud().checkElementConditions(HudElementType.CHAT)) {
     * RenderGameOverlayEvent.Chat event = new
     * RenderGameOverlayEvent.Chat(this.eventParent, 0, this.res.getScaledHeight() -
     * 48); this.chat.setXOffset(event.getPosX());
     * this.chat.setYOffset(event.getPosY()); if
     * (forceRenderType(HudElementType.CHAT)) { if
     * (forceRenderTypeVanilla(HudElementType.CHAT)) { if
     * (!preventElementRenderType(HudElementType.CHAT)) { bind(Gui.ICONS);
     * GlStateManager.enableBlend(); GlStateManager.pushMatrix();
     * GlStateManager.translatef((float) event.getPosX(), (float) event.getPosY(),
     * 0.0F); this.persistantChatGUI.drawChat(this.ticks);
     * GlStateManager.popMatrix(); GlStateManager.disableBlend(); }
     * 
     * if (!preventEventType(HudElementType.CHAT)) { if
     * (MinecraftForge.EVENT_BUS.post(event)) return; post(CHAT); } } else { if
     * (!preventElementRenderType(HudElementType.CHAT)) { bind(Gui.ICONS);
     * GlStateManager.enableBlend();
     * this.rpgHud.getActiveHud().drawElement(HudElementType.CHAT, this,
     * this.zLevel, partialTicks);
     * 
     * GlStateManager.disableBlend(); } if (!preventEventType(HudElementType.CHAT))
     * { if (MinecraftForge.EVENT_BUS.post(event)) return; post(CHAT); } } } else {
     * if (forceRenderTypeVanilla(HudElementType.CHAT)) { if
     * (!preventEventType(HudElementType.CHAT)) { if
     * (MinecraftForge.EVENT_BUS.post(event)) return; } if
     * (!preventElementRenderType(HudElementType.CHAT)) { bind(Gui.ICONS);
     * GlStateManager.enableBlend(); GlStateManager.pushMatrix();
     * GlStateManager.translatef((float) event.getPosX(), (float) event.getPosY(),
     * 0.0F); this.persistantChatGUI.drawChat(this.ticks);
     * GlStateManager.popMatrix(); GlStateManager.disableBlend(); } if
     * (!preventEventType(HudElementType.CHAT)) { post(CHAT); } } else { if
     * (!preventEventType(HudElementType.CHAT)) { if
     * (MinecraftForge.EVENT_BUS.post(event)) return; } if
     * (!preventElementRenderType(HudElementType.CHAT)) { bind(Gui.ICONS);
     * GlStateManager.enableBlend();
