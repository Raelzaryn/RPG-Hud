...
public class NotificationHandler {
	...
	public void onGuiScreenTick(GuiScreenEvent event){
		if(event.gui instanceof GuiMainMenu){
			...
			if(ModRPGHud.instance.settings.getBoolValue(Settings.show_convert_notification) && oldSettingCheck && this.notification == null && (new File(Minecraft.getMinecraft().mcDataDir, "RPGHud_settings.txt").exists() || new File(Minecraft.getMinecraft().mcDataDir, "RPGHud_settings_debug.txt").exists())){
				oldSettingCheck = false;
				this.notification = new NotificationOldSettings(ModRPGHud.instance.settings);
			}
			if(this.notification != null){
				this.notification.drawOnScreen(event.gui);
			}
		}
	}
	...
	public void onMouseInput(GuiScreenEvent.MouseInputEvent event){
		if(this.notification != null){
			...
			if(!Mouse.getEventButtonState() && !shouldFireMouse) shouldFireMouse = true;
			if(this.notification.shouldDestroy) this.notification = null;
		}
		if(!shouldFireMouse && !Mouse.getEventButtonState()) shouldFireMouse = true;
	}
	
	@SubscribeEvent
	public void onActionPerformed(ActionPerformedEvent event){
		if(event.gui instanceof GuiMainMenu && (this.notification != null || !shouldFireMouse)){
