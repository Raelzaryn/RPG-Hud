...
public class HudElementEntityInspectModern extends HudElementEntityInspectVanilla {
    ...
    public void drawElement(DrawContext dc, float zLevel, RenderTickCounter partialTicks, int scaledWidth, int scaledHeight) {
        ...
        if(focused != null) {
            ...
            drawTetragon(dc, posX - 30, posX - 30, 32 + posY, 32 + posY, 90, 76, 10, 10, 0xA0000000);
            drawTetragon(dc, posX - 30, posX - 30, 33 + posY, 33 + posY, 84, 74, 6, 6, 0x20FFFFFF);

            drawTetragon(dc, posX - 30, posX - 30, 33 + posY, 33 + posY, (int) (84 * ((double) health / (double) maxHealth)),
            ...
            if(settings.getBoolValue(Settings.show_entity_armor)) {
                ...
                if(armor > 0) {
                    ...
                    dc.getMatrices().scale(0.5f, 0.5f);
                    dc.drawGuiTexture(RenderPipelines.GUI_TEXTURED, ARMOR_FULL_TEXTURE, (posX - 30) * 2, (posY + 42) * 2, 9, 9);
