...
public class HudElementHealthTexture extends HudElement {
	...
	public void drawElement(DrawContext dc, float zLevel, RenderTickCounter partialTicks, int scaledHeight, int scaledWidth) {
		...
		if (absorption > 1)
			dc.drawTexture(RenderPipelines.GUI_TEXTURED,INTERFACE, posX, posY, 0, 88, (int) (110.0D * ((double) (health + absorption) / (double) (healthMax + absorption))), 12, 256, 256);
		if (this.mc.player.hasStatusEffect(StatusEffects.POISON)) {
			dc.drawTexture(RenderPipelines.GUI_TEXTURED,INTERFACE, posX, posY, 141, 160, (int) (110.0D * ((double) health / (double) (healthMax + absorption))), 12, 256, 256);
		} else if (this.mc.player.hasStatusEffect(StatusEffects.WITHER)) {
			dc.drawTexture(RenderPipelines.GUI_TEXTURED,INTERFACE, posX, posY, 34, 244, (int) (110.0D * ((double) health / (double) (healthMax + absorption))), 12, 256, 256);
		} else {
			dc.drawTexture(RenderPipelines.GUI_TEXTURED,INTERFACE, posX, posY, 0, 100, (int) (110.0D * ((double) health / (double) (healthMax + absorption))), 12, 256, 256);
		}
