...
public class GuiSettingsModColor extends GuiScreenTooltip {
	...
	public GuiSettingsModColor(Screen parent, String color, Text titleIn) {
		...
		setColors();
		this.title = setTitle() + " " + I18n.translate("gui.rpg.editor");
	}

	private String setTitle() {
		return I18n.translate("name." + this.colorType);
	}
	...
	@Override
	public void init() {
		this.addDrawableChild(new GuiSliderMod(GuiSliderMod.EnumColor.RED, this.width / 2 - 75, 40, this.colorR, 0F, 255F, 1F, slider -> slider.onClick(0, 0)));
		this.addDrawableChild(new GuiSliderMod(GuiSliderMod.EnumColor.GREEN, this.width / 2 - 75, 65, this.colorG, 0F, 255F, 1F, slider -> slider.onClick(0, 0)));
		this.addDrawableChild(new GuiSliderMod(GuiSliderMod.EnumColor.BLUE, this.width / 2 - 75, 90, this.colorB, 0F, 255F, 1F, slider -> slider.onClick(0, 0)));

		this.colorCodeField = new TextFieldWidget(client.textRenderer, this.width / 2 - 74, 115, 147, 20, Text.translatable(Settings.intToHexString(this.color, true)));
		this.colorCodeField.setText(Settings.intToHexString(this.color, true));
		...
		for(int i = 0; i < 7; i++) {
			this.addDrawableChild(new GuiButtonTooltip(10 + i,this.width / 4 * 3 - 20, 40 + (i * 20), 60, 20, Text.translatable(colorString[i]), this::actionPerformed));
		}

		for(int i = 0; i < 6; i++) {
			this.addDrawableChild(new GuiButtonTooltip(17 + i, this.width / 4 * 3 + 60 - 20, 40 + (i * 20), 60, 20, Text.translatable(colorString[i+7]), this::actionPerformed));
		}
		...
		}).setTooltip(I18n.translate("tooltip.done")));
		this.addDrawableChild(new GuiButtonTooltip(this.width / 2 + 24, this.height / 6 + 168, 75, 20, Text.translatable("gui.cancel"), button -> client.setScreen(parent)).setTooltip(I18n.translate("tooltip.cancel")));
	}
	...
	public void tick() {
		...
		if (this.colorCodeField.isFocused()) {
			if(!this.colorCodeField.getText().startsWith("#")) {
				StringBuilder s = new StringBuilder("#" + this.colorCodeField.getText());
				if(this.colorCodeField.getText().length() >= 7) {
					s = new StringBuilder("#");
					for(int i = 0; i < 6; i++) {
						s.append(this.colorCodeField.getText().charAt(i));
					}
				}
				this.colorCodeField.setText(s.toString());
			}
	...
	public void render(DrawContext dc, int mouseX, int mouseY, float partialTicks) {
		...
		dc.drawCenteredTextWithShadow(textRenderer, I18n.translate("color.red"), this.width / 2, 40 - 9, -1);
		dc.drawCenteredTextWithShadow(textRenderer, I18n.translate("color.green"), this.width / 2, 65 - 9, -1);
		dc.drawCenteredTextWithShadow(textRenderer, I18n.translate("color.blue"), this.width / 2, 90 - 9, -1);
		this.colorCodeField.render(dc ,mouseX, mouseY, partialTicks);
		dc.drawCenteredTextWithShadow(textRenderer, I18n.translate("gui.rpg.result") + ": " + Settings.intToHexString(this.color, true), this.width / 2, 141, -1);
